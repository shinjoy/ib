﻿﻿document.write("<script src='js/sp/btn.js'></script>");
...
//	},
	...
	onClose: function( selectedDate ) {
		...
		$(this).next().next().datepicker("option","minDate", selectedDate).parent().children('img').css({'vertical-align':'middle', 'line-height':'23px', 'margin-left':'5px'});
	}
};
...
// 고객관리 쓰기 권한에 따른 읽기 권한 체크
var CusFlagChkVal = false;
function CusFlagChk(form, Event) {
	for(var oOo = 0; oOo < form.CusWFlag.length; oOo++) {
		if(form.CusWFlag[oOo].checked && form.CusWFlag[oOo].value == "Y") {
			for(var oOo = 0; oOo < form.CusRFlag.length; oOo++) {
				if(form.CusRFlag[oOo].checked && form.CusRFlag[oOo].value == "N") {
					CusFlagChkVal = true;
					break;
				}
			}
			if(CusFlagChkVal) {	
				for(var oOo = 0; oOo < form.CusRFlag.length; oOo++) {
					if(form.CusRFlag[oOo].value == "Y") form.CusRFlag[oOo].checked = true;
				}
				if(Event == "Edit") alert("고객관리 쓰기권한 부여시 읽기권한도 자동 부여됩니다.");
			}
		}
	}
}
