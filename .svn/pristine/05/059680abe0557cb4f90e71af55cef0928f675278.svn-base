...
<script type="text/JavaScript" src="<c:url value='/js/process.js?ver=1'/>" ></script>
...
<form id="searchCst" name="searchCst" action="<c:url value='/work/popUpCst.do' />" method="post">
	<input type="hidden" id="nameSearch" name="cstNm">
	<input type="hidden" id="nameSearch2" name="cstNmKor">
</form>
			...
			<table class="net_table_apply">
		        ...
		        <tbody>
					...
					<tr class="point">
						...
						<td><input type="text" class="applyinput_B w_st01" id="cst_nm" value="${cstNm}"/></td>
					</tr>
					<tr>
						<th scope="row">회사</th>
						<td>
							<input type="hidden" id="AP_cpnId_0" value="${workVO.cpnId }"/>
					...
					<tr>
						...
						<td>
							<MIDDLE>※ "CEO, 대표, 대표이사" 등의 직급은 모두 <font color="orangered">"대표이사"</font> 선택, 나머지 직급은 직접 입력해주세요.</MIDDLE><br/>
								<label><input type="radio" id="chkCeo" onclick="fnObj.clkPosition('ceo');"> 대표이사</label> | 
								<input class="applyinput_B w_st01" type="text" id="position" onclick="fnObj.clkPosition();" style="ime-mode:active;" />
						</td>
					</tr>
					<tr>
						<th cscope="row"><label for="email"><span class="star">*</span>e-mail</label></th>
						<td><input type="text" id="email"  class="applyinput_B w_st01" style="ime-mode:inactive;"/></td>
					</tr>
					<tr>
						<th scope="row"><label for="phn_1"><span class="star">*</span>휴대전화</label></th>
						<td><input type="text" id="phn_1"  class="applyinput_B w_st01" style="ime-mode:inactive;"/></td>
					</tr>
					<tr>
						<th scope="row"><label for="phn_2">직통전화</label></th>
						<td><input type="text" id="phn_2"   class="applyinput_B w_st01" style="ime-mode:inactive;"/></td>
					</tr>
					<tr>
						<th scope="row"><label for="phn_3">내선전화</label></th>
						<td><input type="text" id="phn_3"  class="applyinput_B w_st01" style="ime-mode:inactive;"/></td>
					</tr>
					<tr>
						...
						<td>
							...
							<span>
								<div style="float:left;font-size:12px;margin-left:2px;margin-top:5px;margin-right:3px;">과의 <b>친밀도</b></div>
								...
								<div style="float:left;font-size:12px;margin-left:7px;margin-top:5px;margin-right:4px;"></div>
								<div><input type="text" name="relMemo" id="relMemo" style="margin-top:3px;width:250px;" class="applyinput_B w_st01" placeholder="관계메모" /></div>
							</span>
						</td>
					</tr>
				</tbody>
...
var fnObj = {
	...
	//	str+=' </optgroup>';
	//	str+=' <optgroup label="FIDES" style="background-color:red;" > '
   /*  	for(var i=0;i<staffList.length;i++){
    		var chk="";
    		if(staffList[i].division=='FIDES'){
    			...
    			str+='<option style="background-color:white;" value='+staffList[i].cusId+chk+'>'+staffList[i].usrNm+'</option>'
    		}
    	}
		str+=' </optgroup>'; */
		str+=' </select>';
    	...
    	var callback = function(result){
			...
			//						$(obj).hide();
							...
							if(page == 1){
				    			...
				    			$('#nameSearch2').val(engTypeToKor(cstNm.val())); //쿼리의 cstNmKor조건 추가, cstNmKor 세팅
