package ib.basic.service;

import ib.work.service.WorkVO;

import java.io.Serializable;

@SuppressWarnings("serial")
public class CpnExcelVO extends WorkVO implements Serializable {

	//일괄 업로드 회사
	private String sNb 					= "";
	private String cpnId 				= ""; 	/* 코드 */
	private String cpnNm 				= "";	/* 법인명 */
	private String market 				= ""; 	/* 시장 */
	private String    ceoId 				= "";	/* CEO ID */
	private String cpnListedCd 			= "";	/* 상장여부 */
	private String categoryBusiness 	= ""; 	/* 업종 */
	private String majorProduct 		= ""; 	/* 주요품목 */
	private String listedDt 			= ""; 	/* 상장일자 */
	private String region				= ""; 	/* 지역 */
	private String capitalization		= ""; 	/* 시가촉액 */
	private String fluctuation			= ""; 	/* 등락율,차입금 */
	private String pbr				 	= ""; 	/* PBR */
	private String per 					= ""; 	/* PER */
	private String share				= ""; 	/* 대주주 지분 */
	private String stock				= ""; 	/* 자사주 */
	private String debtRatio			= ""; 	/* 부채비율 */
	private String cb				 	= ""; 	/* CB */
	private String bw				 	= ""; 	/* BW */
	private String put					= ""; 	/* PUT */
	private String eb					= ""; 	/* EB */
	private String cashable				= ""; 	/* 현금성 */
	private String ir					= ""; 	/* IR */
	private String addr					= ""; 	/* 주소 */
	private String addrDetail			= ""; 	/* 세부주소 */
	private String QnA					= ""; 	/* QnA */
	private String homepage				= ""; 	/* 홈페이지 */
	private String rgId					= ""; 	/* 등록자ID */
	private String upId					= ""; 	/* 수정자ID */
	private String rank					= "";
	
	//일괄 업로드 사람
    private String cstNm = "";
    private String cstSnb = "";
    private String position = "";
    private String categoryPSCd = "";
    private String email = "";
    private String phn1 = "";
    private String phn2 = "";
    private String phn3 = "";
    private String title = "";
    private String note = "";
    private String rgDt = "";
    private String rgNm = "";
    private String upDt = "";
    
    //일괄 업로드 도서
    private String bookNumber = "";
    private String author = "";
    private String publisher = "";
    
    
	/**
	 * @return the sNb
	 */
	public String getsNb() {
		return sNb;
	}
	/**
	 * @param sNb the sNb to set
	 */
	public void setsNb(String sNb) {
		this.sNb = sNb;
	}
	/**
	 * @return the cpnId
	 */
	public String getCpnId() {
		return cpnId;
	}
	/**
	 * @param cpnId the cpnId to set
	 */
	public void setCpnId(String cpnId) {
		this.cpnId = cpnId;
	}
	/**
	 * @return the cpnNm
	 */
	public String getCpnNm() {
		return cpnNm;
	}
	/**
	 * @param cpnNm the cpnNm to set
	 */
	public void setCpnNm(String cpnNm) {
		this.cpnNm = cpnNm;
	}
	/**
	 * @return the market
	 */
	public String getMarket() {
		return market;
	}
	/**
	 * @param market the market to set
	 */
	public void setMarket(String market) {
		this.market = market;
	}
	/**
	 * @return the cpnListedCd
	 */
	public String getCpnListedCd() {
		return cpnListedCd;
	}
	/**
	 * @param cpnListedCd the cpnListedCd to set
	 */
	public void setCpnListedCd(String cpnListedCd) {
		this.cpnListedCd = cpnListedCd;
	}
	/**
	 * @return the categoryBusiness
	 */
	public String getCategoryBusiness() {
		return categoryBusiness;
	}
	/**
	 * @param categoryBusiness the categoryBusinessCd to set
	 */
	public void setCategoryBusiness(String categoryBusiness) {
		this.categoryBusiness = categoryBusiness;
	}
	/**
	 * @return the majorProduct
	 */
	public String getMajorProduct() {
		return majorProduct;
	}
	/**
	 * @param majorProduct the majorProduct to set
	 */
	public void setMajorProduct(String majorProduct) {
		this.majorProduct = majorProduct;
	}
	/**
	 * @return the listedDt
	 */
	public String getListedDt() {
		return listedDt;
	}
	/**
	 * @param listedDt the listedDt to set
	 */
	public void setListedDt(String listedDt) {
		this.listedDt = listedDt;
	}
	/**
	 * @return the region
	 */
	public String getRegion() {
		return region;
	}
	/**
	 * @param region the region to set
	 */
	public void setRegion(String region) {
		this.region = region;
	}
	/**
	 * @return the capitalization
	 */
	public String getCapitalization() {
		return capitalization;
	}
	/**
	 * @param capitalization the capitalization to set
	 */
	public void setCapitalization(String capitalization) {
		this.capitalization = capitalization;
	}
	/**
	 * @return the fluctuation
	 */
	public String getFluctuation() {
		return fluctuation;
	}
	/**
	 * @param fluctuation the fluctuation to set
	 */
	public void setFluctuation(String fluctuation) {
		this.fluctuation = fluctuation;
	}
	/**
	 * @return the pbr
	 */
	public String getPbr() {
		return pbr;
	}
	/**
	 * @param pbr the pbr to set
	 */
	public void setPbr(String pbr) {
		this.pbr = pbr;
	}
	/**
	 * @return the per
	 */
	public String getPer() {
		return per;
	}
	/**
	 * @param per the per to set
	 */
	public void setPer(String per) {
		this.per = per;
	}
	/**
	 * @return the share
	 */
	public String getShare() {
		return share;
	}
	/**
	 * @param share the share to set
	 */
	public void setShare(String share) {
		this.share = share;
	}
	/**
	 * @return the stock
	 */
	public String getStock() {
		return stock;
	}
	/**
	 * @param stock the stock to set
	 */
	public void setStock(String stock) {
		this.stock = stock;
	}
	/**
	 * @return the debtRatio
	 */
	public String getDebtRatio() {
		return debtRatio;
	}
	/**
	 * @param debtRatio the debtRatio to set
	 */
	public void setDebtRatio(String debtRatio) {
		this.debtRatio = debtRatio;
	}
	/**
	 * @return the cb
	 */
	public String getCb() {
		return cb;
	}
	/**
	 * @param cb the cb to set
	 */
	public void setCb(String cb) {
		this.cb = cb;
	}
	/**
	 * @return the bw
	 */
	public String getBw() {
		return bw;
	}
	/**
	 * @param bw the bw to set
	 */
	public void setBw(String bw) {
		this.bw = bw;
	}
	/**
	 * @return the put
	 */
	public String getPut() {
		return put;
	}
	/**
	 * @param put the put to set
	 */
	public void setPut(String put) {
		this.put = put;
	}
	/**
	 * @return the eb
	 */
	public String getEb() {
		return eb;
	}
	/**
	 * @param eb the eb to set
	 */
	public void setEb(String eb) {
		this.eb = eb;
	}
	/**
	 * @return the cashable
	 */
	public String getCashable() {
		return cashable;
	}
	/**
	 * @param cashable the cashable to set
	 */
	public void setCashable(String cashable) {
		this.cashable = cashable;
	}
	/**
	 * @return the ir
	 */
	public String getIr() {
		return ir;
	}
	/**
	 * @param ir the ir to set
	 */
	public void setIr(String ir) {
		this.ir = ir;
	}
	/**
	 * @return the addr
	 */
	public String getAddr() {
		return addr;
	}
	/**
	 * @param addr the addr to set
	 */
	public void setAddr(String addr) {
		this.addr = addr;
	}
	/**
	 * @return the addrDetail
	 */
	public String getAddrDetail() {
		return addrDetail;
	}
	/**
	 * @param addrDetail the addrDetail to set
	 */
	public void setAddrDetail(String addrDetail) {
		this.addrDetail = addrDetail;
	}
	/**
	 * @return the qnA
	 */
	public String getQnA() {
		return QnA;
	}
	/**
	 * @param qnA the qnA to set
	 */
	public void setQnA(String qnA) {
		QnA = qnA;
	}
	/**
	 * @return the homepage
	 */
	public String getHomepage() {
		return homepage;
	}
	/**
	 * @param homepage the homepage to set
	 */
	public void setHomepage(String homepage) {
		this.homepage = homepage;
	}
	/**
	 * @return the rgId
	 */
	public String getRgId() {
		return rgId;
	}
	/**
	 * @param rgId the rgId to set
	 */
	public void setRgId(String rgId) {
		this.rgId = rgId;
	}
	/**
	 * @return the upId
	 */
	public String getUpId() {
		return upId;
	}
	/**
	 * @param upId the upId to set
	 */
	public void setUpId(String upId) {
		this.upId = upId;
	}
	/**
	 * @return the cstNm
	 */
	public String getCstNm() {
		return cstNm;
	}
	/**
	 * @param cstNm the cstNm to set
	 */
	public void setCstNm(String cstNm) {
		this.cstNm = cstNm;
	}
	/**
	 * @return the position
	 */
	public String getPosition() {
		return position;
	}
	/**
	 * @param position the position to set
	 */
	public void setPosition(String position) {
		this.position = position;
	}
	/**
	 * @return the categoryPSCd
	 */
	public String getCategoryPSCd() {
		return categoryPSCd;
	}
	/**
	 * @param categoryPSCd the categoryPSCd to set
	 */
	public void setCategoryPSCd(String categoryPSCd) {
		this.categoryPSCd = categoryPSCd;
	}
	/**
	 * @return the email
	 */
	public String getEmail() {
		return email;
	}
	/**
	 * @param email the email to set
	 */
	public void setEmail(String email) {
		this.email = email;
	}
	/**
	 * @return the phn1
	 */
	public String getPhn1() {
		return phn1;
	}
	/**
	 * @param phn1 the phn1 to set
	 */
	public void setPhn1(String phn1) {
		this.phn1 = phn1;
	}
	/**
	 * @return the phn2
	 */
	public String getPhn2() {
		return phn2;
	}
	/**
	 * @param phn2 the phn2 to set
	 */
	public void setPhn2(String phn2) {
		this.phn2 = phn2;
	}
	/**
	 * @return the phn3
	 */
	public String getPhn3() {
		return phn3;
	}
	/**
	 * @param phn3 the phn3 to set
	 */
	public void setPhn3(String phn3) {
		this.phn3 = phn3;
	}
	/**
	 * @return the rgDt
	 */
	public String getRgDt() {
		return rgDt;
	}
	/**
	 * @param rgDt the rgDt to set
	 */
	public void setRgDt(String rgDt) {
		this.rgDt = rgDt;
	}
	/**
	 * @return the rgNm
	 */
	public String getRgNm() {
		return rgNm;
	}
	/**
	 * @param rgNm the rgNm to set
	 */
	public void setRgNm(String rgNm) {
		this.rgNm = rgNm;
	}
	/**
	 * @return the upDt
	 */
	public String getUpDt() {
		return upDt;
	}
	/**
	 * @param upDt the upDt to set
	 */
	public void setUpDt(String upDt) {
		this.upDt = upDt;
	}
	/**
	 * @return the note
	 */
	public String getNote() {
		return note;
	}
	/**
	 * @param note the note to set
	 */
	public void setNote(String note) {
		this.note = note;
	}
	/**
	 * @return the cstSnb
	 */
	public String getCstSnb() {
		return cstSnb;
	}
	/**
	 * @param cstSnb the cstSnb to set
	 */
	public void setCstSnb(String cstSnb) {
		this.cstSnb = cstSnb;
	}
	/**
	 * @return the title
	 */
	public String getTitle() {
		return title;
	}
	/**
	 * @param title the title to set
	 */
	public void setTitle(String title) {
		this.title = title;
	}
	/**
	 * @return the rank
	 */
	public String getRank() {
		return rank;
	}
	/**
	 * @param rank the rank to set
	 */
	public void setRank(String rank) {
		this.rank = rank;
	}
	/**
	 * @return the bookNumber
	 */
	public String getBookNumber() {
		return bookNumber;
	}
	/**
	 * @param bookNumber the bookNumber to set
	 */
	public void setBookNumber(String bookNumber) {
		this.bookNumber = bookNumber;
	}
	/**
	 * @return the author
	 */
	public String getAuthor() {
		return author;
	}
	/**
	 * @param author the author to set
	 */
	public void setAuthor(String author) {
		this.author = author;
	}
	/**
	 * @return the publisher
	 */
	public String getPublisher() {
		return publisher;
	}
	/**
	 * @param publisher the publisher to set
	 */
	public void setPublisher(String publisher) {
		this.publisher = publisher;
	}
	/**
	 * @return the ceoId
	 */
	public String getCeoId() {
		return ceoId;
	}
	/**
	 * @param ceoId the ceoId to set
	 */
	public void setCeoId(String ceoId) {
		this.ceoId = ceoId;
	}
 

}