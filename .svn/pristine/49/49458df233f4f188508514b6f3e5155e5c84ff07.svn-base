...
<input type="hidden" id="rgstId" value="<c:out value='${userLoginInfo.id}'/>">
	<section>
		...
		<form name="modifyRec" action="<c:url value='/work/selectWorkResale.do' />" method="post"><input type="hidden" name="sorting" id="sorting"><input type="hidden" id="tt1" name="total" value="${ttT}"></form>
		<input type="hidden" id="SendReceive" value="RS">
		
		<div class="fixed-top" style="z-index:10;">

			<header>
				<form name="dayForm" id="dayForm" method="post" action="<c:url value='/work/selectWorkResale.do' />">
...
<%-- 					
 ...
 --%>
 						<th rowspan="2" onclick="sortTable();" class="hand bgYlw" >재매각일자<br/>▼</th>
						<th rowspan="2" onclick="sortTable(1);" class="hand bgYlw">소개자<br/>▼</th>
						<th rowspan="2" onclick="sortTable(3);" class="hand bgYlw">제안자<br/>▼</th>
						<th colspan="2" class="bgYlw">제안받은자<small> (고객)</small></th>
						<th rowspan="2" onclick="sortTable(4);" class="hand bgYlw">물건<small><br>(회사명)</small><br/>▼</th>
						<th rowspan="2" class="bgYlw">발행규모</th>
						<th rowspan="2" class="bgYlw">기한</th>
						<th rowspan="2" class="bgYlw">피드백</th>
						<th rowspan="2" class="bgYlw">진행사항</th>
						<th rowspan="2" class="bgYlw">성과</th>
						<th rowspan="2" class="bgYlw">메모</th>
					</tr>
					<tr>
						<th class="bgYlw">회사</th>
						<th onclick="sortTable(2);" class="hand bgYlw">이름<br>▼</th>
				...
				<tbody>
					<c:forEach var="offer" items="${offerList}" varStatus="status">
						...
						>
							...
							<td class="cent"<c:if test="${fn:length(offer.price) > 7}">style="width:60px;"</c:if>>${offer.price}</td>
							<td>
<!-- ------------------------------------------------ -->
...
<!-- ------------------------------------------------ -->
							...
							<td>
								...
								<c:choose>
									...
									<c:otherwise>${fn:substring(offer.result,0,16)}<c:if test="${fn:length(offer.result) > 16}">...</c:if></c:otherwise>
								</c:choose>
								</a>
							</td>
							<td>
