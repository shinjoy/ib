...
var modifyBook = function(th){
	...
	var Title = obj.find('[id^=bookTitle]').val();
	var Author = obj.find('[id^=author]').val();
	var Epil = obj.find('[id^=epilogue]').val();
...
<div id="wrap">
	...
	<c:forEach var="book" items="${bookList}" varStatus="bookS">
		<div class="popUpMenu" id="staffL${bookS.count}">
			...
			<p class="bsnsR_btn">
				<c:if test="${userLoginInfo.permission > '00019'}"><span class="btn s green"><a id="saveReader${bookS.count}" onclick="saveReader()">&nbsp;순위지정&nbsp;</a></span></c:if>
			</p>
		</div>
	</c:forEach>
				...
				<table class="t_skinR00" style="width:100%">
					<tbody><c:set var="subBegin" value="0"/>
						...
						<tr <c:if test="${not empty book.rent}">class="bgRed"</c:if>>
							<td class="cent w70p" id="bookNumber${bookS.count}"><b>${book.bookNum}</b><c:if test="${userLoginInfo.name==book.usrNm}">
							...
							<td class="cent pd0" colspan="3">
								<%-- 
								<c:if test="${userLoginInfo.name==book.usrNm}">
								...
								<c:forEach var="ep" items="${epilogueList}" begin="${subBegin}" varStatus="epS"><c:if test="${book.bookNum == ep.bookNum}">
									<tr>
										<td class="cent w70p bd0">${ep.rgDt}<c:if test="${userLoginInfo.name==ep.usrNm}">
