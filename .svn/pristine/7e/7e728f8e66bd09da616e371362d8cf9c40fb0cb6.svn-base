...
<html>
	...
	<title>Synergy Partners</title>
		...
		<script type="text/javaScript" language="javascript">
			...
			// 목록
			function BoardList() {
				$('#HolidayView').attr('action', "<c:url value='/BoardList.do'/>").submit();
			}
			
			// 결재//반송 목록
			function PopList() {
				if($('#CMD').val() == 'ConfirmList') $('#HolidayView').attr('action', "<c:url value='/ConfirmList.do'/>").submit();
				else $('#HolidayView').attr('action', "<c:url value='/ReturnList.do'/>").submit();
			}
	...
	<input type="hidden" name="HoliCancelCd"	id="HoliCancelCd"		value="${BoardInfo.holiCancelCd}"/>
	<table class="subtable" border="0" cellpadding="0" cellspacing="0" width="1024">
		...
		<tr>
			<td class="input_search Text_L" colspan="4" style="size:8pt;">
				...
				<a href="javascript:URLCopy();"><img src="<c:url value='/images/sp/base/copy.gif'/>" border="0" align="middle"></a>
		...
		<tr>
			...
			<td class="input_B Text_L" colspan="3" style="padding:5px;">${fn:replace(BoardInfo.writeCon, cn, br)}</td>
		</tr>
		<c:if test="${BoardInfo.confirmProcFlag eq 'Y' && BoardInfo.confirmPerSabun ne ''}">
			<tr>
				<td class="desc_BR Text_C">최종승인자</td>
				<td class="input_B Text_L" colspan="3" style="padding:5px;line-height:20px;">${BoardInfo.confirmPerNm}</td>
			</tr>
			<tr> 
		...
		<tr>
			...
			<c:forEach var="result" items="${ReplyList}" varStatus="status">
				...
				<tr>
					...
					<td class="input_T Text_L" id="Con${result.replycd}">${fn:replace(result.replycon, cn, br)}</td>
