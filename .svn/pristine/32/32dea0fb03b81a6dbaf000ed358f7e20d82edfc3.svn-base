	...
	$("a.title").bind('dropstart', function(event){
		...
		if(splitClic[1]!=splitThis[1]){
			...
			var DATA='',url=''
				...
				,date = $("#choice_year").val()+$("#choice_month").val()+$(this).attr('name')
			...
			if("<c:out value='${userLoginInfo.name}'/>" == "<c:out value='${workVO.name}'/>" | "<c:out value='${userLoginInfo.permission}' />" > "00019"){
		...
		if(select1st.val()=='00012'){//정보
			...
			switch(obj.val()){
				...
				case '00003':
					...
					//회사 와 회사관계자 위치 바꿈
					$('span[id^=slctCpn_]').insertAfter('span[id^=slctPrsn_]');
...
$(document).on("click",'.offerR_btnOk',function(){
	...
	var getCpnNm = obj.find('[id^=AP_cpnNm]').text();
	...
	var getCategoryNm = obj.find('[id^=categoryCd] option:selected').text();
	...
	var cpnListed = obj.find('[id^=AP_cpnListed]').val();		//상장사 여부
