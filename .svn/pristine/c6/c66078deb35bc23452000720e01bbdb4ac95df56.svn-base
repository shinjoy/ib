...
<html>
	...
	<title>Synergy Partners</title>
		...
		<script type="text/javaScript" language="javascript">
			...
			$(document).ready(function () {
				$(window).load(function() {
					...
					$('#WinClose').click(function() {
						if($('#WinClose').is(':checked')) setWinCookie('confirm', 'Y', 1);
						self.close();
					});
					BtnSet();
				});
			...
			function BoardView(GrpCd, CateCd, WriteCd) {
				...
				$('#CateCd').val(CateCd);
				$('#WriteCd').val(WriteCd);
				$('#ConfirmList').attr('action', "<c:url value='/BoardView.do'/>").submit();
			}
			-->
		</script>
	</head>
	<body>
	<form name="ConfirmList" id="ConfirmList" action="<c:url value='/ConfirmList.do'/>" method="post">
	<input type="hidden" name="InfoMessage"		id="InfoMessage"/>
	<input type="hidden" name="message"			id="message"			value="${message}"/>
	<input type="hidden" name="CMD"				id="CMD"				value="ConfirmList"/>
	...
	<table class="subtable" border="0" cellpadding="0" cellspacing="0" width="99%" align="center">
		<tr>
			<td class="Title_TB Text_L" colspan="2" style="height:40px;"><img src="images/sp/arrow3.gif" style="line-height:23px;vertical-align:middle;"> 결재 진행중 문서</td>
		</tr>	
		<tr>
			<td colspan="2">
			<table class="form3" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tr class="Text_C Click">
					...
					<td class="grid_title" style="width:15%;" id="Order1" onclick="javascript:ListOrderSet('ConfirmList.do', '_self', 'ConfirmList', this.id, '보고서명', 'CateNm', 'S');">보고서명(▲)</td>
					<td class="grid_title" style="width:*;" id="Order2" onclick="javascript:ListOrderSet('ConfirmList.do', '_self', 'ConfirmList', this.id, '제목', 'WriteTitle', 'S');">제목(▲)</td>
					<td class="grid_title" style="width:5%;">파일</td>
					<td class="grid_title" style="width:10%;" id="Order3" onclick="javascript:ListOrderSet('ConfirmList.do', '_self', 'ConfirmList', this.id, '작성일', 'RegDate', 'S');">작성일(▲)</td>
					<td class="grid_title" style="width:10%;" id="Order4" onclick="javascript:ListOrderSet('ConfirmList.do', '_self', 'ConfirmList', this.id, '작성자', 'RegPerNm', 'S');">작성자(▲)</td>
				</tr>
			</table>
			</td>
		</tr>
	...
	<table class="form3_noline" border="0" cellpadding="0" cellspacing="0" width="99%" align="center">
		<c:if test="${fn:length(ConfirmIngList) eq 0}"><tr><td style="text-align:center;width:100%;height:50px;">검색된 게시물이 없습니다.</td></tr></c:if>
		<c:forEach var="result" items="${ConfirmIngList}" varStatus="status">
			...
			<tr class="Text_C Click" onclick="javascript:BoardView('${result.grpcd}', '${result.catecd}', '${result.writecd}');" ${CellColor}>
				...
				<td class="grid_input_BR" style="width:10%;">${result.regpernm}</td>
			</tr>
		</c:forEach>
		<tr>
