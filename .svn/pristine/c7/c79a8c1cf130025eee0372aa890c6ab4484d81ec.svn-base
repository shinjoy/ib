package ib.schedule.service;

import ib.work.service.WorkVO;


@SuppressWarnings("serial")
public class PersonVO  extends WorkVO{
	
	private String sNb = "";
    private String cstNm = "";
    private String cpnNm = "";
    private String cpnId = "";
    private String position = "";
    private String categoryPSNm = "";
    private String categoryPSCd = "";
    private String email = "";
    private String phn1 = "";
    private String phn2 = "";
    private String phn3 = "";
    private String lvCd = "";
    private String rgDt = "";
    private String rgId = "";
    private String rgNm = "";
    private String upDt = "";
    private String upId = "";
    private String sabun = "";
    private String cpnSnb = "";
    
    private String TMname = "";
    private String TMsnb = "";
    private String sNb1st = "";
    private String sNb2nd = "";
    private String netYN = "";
    private String flag = "";
    
    private String cstSnb = "";
    private String title = "";
    private String note = "";
    
    private String fileUrl = "";
    private String sort_t = "";
    
    private String searchCstNm = "";
    private String choose_popMain = "";
    
    private String popUp = "";
    private String cate = "";
    private String RegPerSabun = "";
    
    private String meetNm = "";
    private String phnExposure = "";
    
    
    private String netKnd;			//인맥구분
	private String cpnDeptCd;		//부서코드
	private String cpnDeptNm;		//부서명
    
	private String usrCusId = "";	//고객(인물) 담당자 cusId
    private String relDegree;		//담당자와의 친밀도
    private String relMemo;			//담당자와의 관계메모
    
    private String cstNmKor = "";	//한글명 (영어타이핑 > 한글변환..지원)
    
	
	/**
	 * @return the sNb
	 */
	public String getsNb() {
		return sNb;
	}
	/**
	 * @param sNb the sNb to set
	 */
	public void setsNb(String sNb) {
		this.sNb = sNb;
	}
	/**
	 * @return the cstNm
	 */
	public String getCstNm() {
		return cstNm;
	}
	/**
	 * @param cstNm the cstNm to set
	 */
	public void setCstNm(String cstNm) {
		this.cstNm = cstNm;
	}
	/**
	 * @return the position
	 */
	public String getPosition() {
		return position;
	}
	/**
	 * @param position the position to set
	 */
	public void setPosition(String position) {
		this.position = position;
	}
	/**
	 * @return the categoryPSCd
	 */
	public String getCategoryPSCd() {
		return categoryPSCd;
	}
	/**
	 * @param categoryPSCd the categoryPSCd to set
	 */
	public void setCategoryPSCd(String categoryPSCd) {
		this.categoryPSCd = categoryPSCd;
	}
	/**
	 * @return the rgDt
	 */
	public String getRgDt() {
		return rgDt;
	}
	/**
	 * @param rgDt the rgDt to set
	 */
	public void setRgDt(String rgDt) {
		this.rgDt = rgDt;
	}
	/**
	 * @return the rgId
	 */
	public String getRgId() {
		return rgId;
	}
	/**
	 * @param rgId the rgId to set
	 */
	public void setRgId(String rgId) {
		this.rgId = rgId;
	}
	/**
	 * @return the upDt
	 */
	public String getUpDt() {
		return upDt;
	}
	/**
	 * @param upDt the upDt to set
	 */
	public void setUpDt(String upDt) {
		this.upDt = upDt;
	}
	/**
	 * @return the upId
	 */
	public String getUpId() {
		return upId;
	}
	/**
	 * @param upId the upId to set
	 */
	public void setUpId(String upId) {
		this.upId = upId;
	}
	/**
	 * @return the email
	 */
	public String getEmail() {
		return email;
	}
	/**
	 * @param email the email to set
	 */
	public void setEmail(String email) {
		this.email = email;
	}
	/**
	 * @return the phn1
	 */
	public String getPhn1() {
		return phn1;
	}
	/**
	 * @param phn1 the phn1 to set
	 */
	public void setPhn1(String phn1) {
		this.phn1 = phn1;
	}
	/**
	 * @return the phn2
	 */
	public String getPhn2() {
		return phn2;
	}
	/**
	 * @param phn2 the phn2 to set
	 */
	public void setPhn2(String phn2) {
		this.phn2 = phn2;
	}
	/**
	 * @return the phn3
	 */
	public String getPhn3() {
		return phn3;
	}
	/**
	 * @param phn3 the phn3 to set
	 */
	public void setPhn3(String phn3) {
		this.phn3 = phn3;
	}
	/**
	 * @return the cpnId
	 */
	public String getCpnId() {
		return cpnId;
	}
	/**
	 * @param cpnId the cpnId to set
	 */
	public void setCpnId(String cpnId) {
		this.cpnId = cpnId;
	}
	/**
	 * @return the cpnNm
	 */
	public String getCpnNm() {
		return cpnNm;
	}
	/**
	 * @param cpnNm the cpnNm to set
	 */
	public void setCpnNm(String cpnNm) {
		this.cpnNm = cpnNm;
	}
	/**
	 * @return the sNb1st
	 */
	public String getsNb1st() {
		return sNb1st;
	}
	/**
	 * @param sNb1st the sNb1st to set
	 */
	public void setsNb1st(String sNb1st) {
		this.sNb1st = sNb1st;
	}
	/**
	 * @return the sNb2nd
	 */
	public String getsNb2nd() {
		return sNb2nd;
	}
	/**
	 * @param sNb2nd the sNb2nd to set
	 */
	public void setsNb2nd(String sNb2nd) {
		this.sNb2nd = sNb2nd;
	}
	/**
	 * @return the cstSnb
	 */
	public String getCstSnb() {
		return cstSnb;
	}
	/**
	 * @param cstSnb the cstSnb to set
	 */
	public void setCstSnb(String cstSnb) {
		this.cstSnb = cstSnb;
	}
	/**
	 * @return the title
	 */
	public String getTitle() {
		return title;
	}
	/**
	 * @param title the title to set
	 */
	public void setTitle(String title) {
		this.title = title;
	}
	/**
	 * @return the note
	 */
	public String getNote() {
		return note;
	}
	/**
	 * @param note the note to set
	 */
	public void setNote(String note) {
		this.note = note;
	}
	/**
	 * @return the rgNm
	 */
	public String getRgNm() {
		return rgNm;
	}
	/**
	 * @param rgNm the rgNm to set
	 */
	public void setRgNm(String rgNm) {
		this.rgNm = rgNm;
	}
	/**
	 * @return the fileUrl
	 */
	public String getFileUrl() {
		return fileUrl;
	}
	/**
	 * @param fileUrl the fileUrl to set
	 */
	public void setFileUrl(String fileUrl) {
		this.fileUrl = fileUrl;
	}
	/**
	 * @return the sort_t
	 */
	public String getSort_t() {
		return sort_t;
	}
	/**
	 * @param sort_t the sort_t to set
	 */
	public void setSort_t(String sort_t) {
		this.sort_t = sort_t;
	}
	/**
	 * @return the searchCstNm
	 */
	public String getSearchCstNm() {
		return searchCstNm;
	}
	/**
	 * @param searchCstNm the searchCstNm to set
	 */
	public void setSearchCstNm(String searchCstNm) {
		this.searchCstNm = searchCstNm;
	}
	/**
	 * @return the choose_popMain
	 */
	public String getChoose_popMain() {
		return choose_popMain;
	}
	/**
	 * @param choose_popMain the choose_popMain to set
	 */
	public void setChoose_popMain(String choose_popMain) {
		this.choose_popMain = choose_popMain;
	}
	/**
	 * @return the tMname
	 */
	public String getTMname() {
		return TMname;
	}
	/**
	 * @param tMname the tMname to set
	 */
	public void setTMname(String tMname) {
		TMname = tMname;
	}
	/**
	 * @return the tMsnb
	 */
	public String getTMsnb() {
		return TMsnb;
	}
	/**
	 * @param tMsnb the tMsnb to set
	 */
	public void setTMsnb(String tMsnb) {
		TMsnb = tMsnb;
	}
	/**
	 * @return the netYN
	 */
	public String getNetYN() {
		return netYN;
	}
	/**
	 * @param netYN the netYN to set
	 */
	public void setNetYN(String netYN) {
		this.netYN = netYN;
	}
	/**
	 * @return the sabun
	 */
	public String getSabun() {
		return sabun;
	}
	/**
	 * @param sabun the sabun to set
	 */
	public void setSabun(String sabun) {
		this.sabun = sabun;
	}
	/**
	 * @return the popUp
	 */
	public String getPopUp() {
		return popUp;
	}
	/**
	 * @param popUp the popUp to set
	 */
	public void setPopUp(String popUp) {
		this.popUp = popUp;
	}
	/**
	 * @return the cate
	 */
	public String getcate() {
		return cate;
	}
	/**
	 * @param cate the cate to set
	 */
	public void setcate(String cate) {
		this.cate = cate;
	}
	/**
	 * @return the regPerSabun
	 */
	public String getRegPerSabun() {
		return RegPerSabun;
	}
	/**
	 * @param regPerSabun the regPerSabun to set
	 */
	public void setRegPerSabun(String regPerSabun) {
		RegPerSabun = regPerSabun;
	}
	/**
	 * @return the lvCd
	 */
	public String getLvCd() {
		return lvCd;
	}
	/**
	 * @param lvCd the lvCd to set
	 */
	public void setLvCd(String lvCd) {
		this.lvCd = lvCd;
	}
	/**
	 * @return the flag
	 */
	public String getFlag() {
		return flag;
	}
	/**
	 * @param flag the flag to set
	 */
	public void setFlag(String flag) {
		this.flag = flag;
	}
	/**
	 * @return the cpnSnb
	 */
	public String getCpnSnb() {
		return cpnSnb;
	}
	/**
	 * @param cpnSnb the cpnSnb to set
	 */
	public void setCpnSnb(String cpnSnb) {
		this.cpnSnb = cpnSnb;
	}
	/**
	 * @return the categoryPSNm
	 */
	public String getCategoryPSNm() {
		return categoryPSNm;
	}
	/**
	 * @param categoryPSNm the categoryPSNm to set
	 */
	public void setCategoryPSNm(String categoryPSNm) {
		this.categoryPSNm = categoryPSNm;
	}
	/**
	 * @return the meetNm
	 */
	public String getMeetNm() {
		return meetNm;
	}
	/**
	 * @param meetNm the meetNm to set
	 */
	public void setMeetNm(String meetNm) {
		this.meetNm = meetNm;
	}
	/**
	 * @return the phnExposure
	 */
	public String getPhnExposure() {
		return phnExposure;
	}
	/**
	 * @param phnExposure the phnExposure to set
	 */
	public void setPhnExposure(String phnExposure) {
		this.phnExposure = phnExposure;
	}
	
	
	public String getNetKnd() {
		return netKnd;
	}
	public void setNetKnd(String netKnd) {
		this.netKnd = netKnd;
	}
	public String getCpnDeptCd() {
		return cpnDeptCd;
	}
	public void setCpnDeptCd(String cpnDeptCd) {
		this.cpnDeptCd = cpnDeptCd;
	}
	public String getCpnDeptNm() {
		return cpnDeptNm;
	}
	public void setCpnDeptNm(String cpnDeptNm) {
		this.cpnDeptNm = cpnDeptNm;
	}
	public String getUsrCusId() {
		return usrCusId;
	}
	public void setUsrCusId(String usrCusId) {
		this.usrCusId = usrCusId;
	}
	public String getRelDegree() {
		return relDegree;
	}
	public void setRelDegree(String relDegree) {
		this.relDegree = relDegree;
	}
	public String getRelMemo() {
		return relMemo;
	}
	public void setRelMemo(String relMemo) {
		this.relMemo = relMemo;
	}
	public String getCstNmKor() {
		return cstNmKor;
	}
	public void setCstNmKor(String cstNmKor) {
		this.cstNmKor = cstNmKor;
	}
	
	
}