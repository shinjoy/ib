...
<input type="hidden" id="rgstId" value="<c:out value='${userLoginInfo.id}'/>">
	...
	<header>
		...
		<div class="year_wrap">
			...
			<a <c:if test="${choice_month == '01' }">class="red bold"</c:if> onclick="month('01','monthTardiness.do')">1월</a><span> | </span>
			<a <c:if test="${choice_month == '02' }">class="red bold"</c:if> onclick="month('02','monthTardiness.do')">2월</a><span> | </span>
			<a <c:if test="${choice_month == '03' }">class="red bold"</c:if> onclick="month('03','monthTardiness.do')">3월</a><span> | </span>
			<a <c:if test="${choice_month == '04' }">class="red bold"</c:if> onclick="month('04','monthTardiness.do')">4월</a><span> | </span>
			<a <c:if test="${choice_month == '05' }">class="red bold"</c:if> onclick="month('05','monthTardiness.do')">5월</a><span> | </span>
			<a <c:if test="${choice_month == '06' }">class="red bold"</c:if> onclick="month('06','monthTardiness.do')">6월</a><span> | </span>
			<a <c:if test="${choice_month == '07' }">class="red bold"</c:if> onclick="month('07','monthTardiness.do')">7월</a><span> | </span>
			<a <c:if test="${choice_month == '08' }">class="red bold"</c:if> onclick="month('08','monthTardiness.do')">8월</a><span> | </span>
			<a <c:if test="${choice_month == '09' }">class="red bold"</c:if> onclick="month('09','monthTardiness.do')">9월</a><span> | </span>
			<a <c:if test="${choice_month == '10' }">class="red bold"</c:if> onclick="month('10','monthTardiness.do')">10월</a><span> | </span>
			<a <c:if test="${choice_month == '11' }">class="red bold"</c:if> onclick="month('11','monthTardiness.do')">11월</a><span> | </span>
			<a <c:if test="${choice_month == '12' }">class="red bold"</c:if> onclick="month('12','monthTardiness.do')">12월</a>
		</div>
		</form>
	</header>
	<section>
			...
			<table class="t_skin05 t_skin05width">
				<thead>
					<tr>
						...
						<th rowspan="2" class="bgYlw">총합계</th>
					</tr>
			...
			<form name="insertCar" id="insertCar" method="post">
				<tbody>
					...
					<tr>
						...
						<td class="cent <c:if test="${staff.usrId==userLoginInfo.id}">bgGrn</c:if>" id="${staff.sNb}_sumCnt">&nbsp;</td>
						<td class="cent <c:if test="${staff.usrId==userLoginInfo.id}">bgGrn</c:if>" id="${staff.sNb}_totSumCnt">&nbsp;</td>
					</tr>
					</c:if>
					</c:forEach>
				</tbody>
		...
		<c:forEach var="MonList" items="${personnelMonthList }" varStatus="monStatus">
			<input type="hidden" class="viewPersonnel" id="${MonList.staffId}_data_${MonList.tmDt}" value="${fn:substring(MonList.holiday,0,2)}:${fn:substring(MonList.holiday,2,4)}">
		</c:forEach>
