		...
		<c:forEach var="idea" items="${ideaList}" varStatus="ideaS">
			<div class="popUpMenu title_area" id="idea${ideaS.count}" style="z-index:99;">
				...
				<ul><input type="hidden" id="snbIdea${ideaS.count}" value="${idea.sNb}" />
					...
					<span class="bold" style="display:inline-block;">구분 :&nbsp;</span><select id="cd${ideaS.count}" class="w80p" <c:if test="${userLoginInfo.id!=idea.rgId}">disabled</c:if>>
					...
					<input type="text" id="title${ideaS.count}" value="${idea.title}" style="width:323px" <c:if test="${userLoginInfo.id!=idea.rgId}">disabled</c:if>/>
					<c:if test="${userLoginInfo.id==idea.rgId}"><span class="btn s orange" onclick="viewStaff('test',this,event);"><a>참가자</a></span></c:if></li>
					<c:if test="${userLoginInfo.id==idea.rgId}"><li class="c_note"><textarea id="ideaC${ideaS.count}">${idea.idea}</textarea></li></c:if>
					<c:if test="${userLoginInfo.id!=idea.rgId}"><li class="c_note v-textarea" style="font-size:13px !important;line-height: 150%;color:black;">${fn:replace(idea.idea, lf,"<br/>")}</li></c:if>
					<c:if test="${userLoginInfo.id==idea.rgId}"><li class="bsnsR_btn">
		...
		<c:forEach var="rcmd" items="${rcmdList}" varStatus="rcmdS">
			<div class="popUpMenu title_area" id="rcmdReasn${rcmdS.count}">
				...
				<ul><input type="hidden" id="snbRcmd${rcmdS.count}" value="${rcmd.sNb}" />
					<c:if test="${userLoginInfo.id==rcmd.rgId}"><li class="c_note"><textarea id="reason${rcmdS.count}" style="font-size:13px !important;line-height: 150%;color:black;">${rcmd.reason}</textarea></li></c:if>
					<c:if test="${userLoginInfo.id!=rcmd.rgId}"><li class="c_note v-textarea" style="font-size:13px !important;line-height: 150%;color:black;">${fn:replace(rcmd.reason, lf,"<br/>")}</li></c:if>
					<c:if test="${userLoginInfo.id==rcmd.rgId}"><li class="bsnsR_btn">
		...
		<form name="insertBook" id="insertBook" method="post">
			<table class="t_skinR00" style="width:100%;table-layout:fixed;">
				...
				<tbody><c:set var="subBegin" value="0"/>
					...
					>
						<td class="cent" id="bookNumber${ideaS.count}"><b>${idea.rgDt}</b><br/><nobr><c:if test="${userLoginInfo.name==idea.usrNm}">
							...
							<c:if test="${userLoginInfo.id=='gjh'}"><span class="btn s gold"><a onclick="view('score${ideaS.count}')">평가</a></span></c:if>
						</td>
						...
						<td class="cent rcmdRs rcmdTableTd">
							...
							<table class="rcmdRs rcmdTable" style="float:left;width:100%;">
								<tr>
									...
									<c:if test="${userLoginInfo.permission > '00019'}"><td style="width:57px;vertical-align:top;">
						...
						<td class="cent">
							<select class="processCd" id="processCd_${idea.sNb}"><c:forEach var="cmmCd" items="${cmmCdProcessList}" varStatus="prcsS">
								<option value="${cmmCd.dTailCd}" <c:if test="${idea.process == cmmCd.dTailCd}">selected</c:if>>${cmmCd.cdNm}</option>
							</c:forEach>
							</select>
						</td>
