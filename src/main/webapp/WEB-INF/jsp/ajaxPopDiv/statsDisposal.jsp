	...
	<c:forEach var="dsps" items="${disposalList}" varStatus="dspsS"><c:set var="sumRspt1" value="0"/>
		...
		<c:if test="${dsps.offerSnb != saveSnb}">
			...
			<c:if test="${dspsS.count>1}"><div class="boxBottom"></div></div></div><div class="boxBottom"></div></c:if>
				<a href="javascript:popUp('${dsps.offerTmDt}','rcmdComment','','${dsps.offerSnb}');"><b>${dsps.cpnNm}</b> ${dsps.categoryNm} <font style="color:pink;font-size:15px;"><i class="axi axi-pageview staffNm_dtl_view"></i></font></a>
					...
					<div>
						...
						<div class="midboxcontents">
							...
							<div class="price" style="width:50px;">
								...
								<c:set var="priceTotal" value="${priceTotal + ((dsps.priceNum * 1 * (100-sumRspt1)) / 100)}" />		<!-- 하단 총합계 위해 sum -->
								<c:set var="marginTotal" value="${marginTotal + ((dsps.marginNum * 1 * (100-sumRspt1)) / 100)}" />	<!-- 하단 총합계 위해 sum -->
				...
				총계 : 
				<fmt:formatNumber var="priceTotal" value="${priceTotal}" pattern="0.0"/>
				<fmt:formatNumber var="marginTotal" value="${marginTotal}" pattern="0.0"/>
				<fmt:formatNumber var="gainTotal" value="${marginTotal*1 - priceTotal*1}" pattern="0.0"/>
				${priceTotal} / ${marginTotal} <br/>
				마진 : ${gainTotal}
			</li>
			<li class="bsnsR_btn">
