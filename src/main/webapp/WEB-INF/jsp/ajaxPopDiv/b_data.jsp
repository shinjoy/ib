...
<!-- ------- icon css (by font) :S ------- -->	
<link rel="stylesheet" type="text/css" href="<c:url value='/fonts/awesome/font-awesome.min.local.css'/>"><!-- font icon awesome -->
<link rel="stylesheet" type="text/css" href="<c:url value='/fonts/axicon/axicon.min.css'/>" /><!-- font icon axicon -->	
<!-- ------- icon css (by font) :E ------- -->
...
.tab_t_on{
    ...
    color: #ff5b49;
    border-top: 1px solid #ff4b49;
    border-left: 1px solid #ff4b49;
    border-right: 1px solid #ff4b49;
    font-weight: 700;
    box-shadow: 0px 2px 6px #ff5b49;
 }
.tooltip1{
	width: 500px;
    margin-left: 200px;
...
<script type="text/JavaScript" src="${pageContext.request.contextPath}/js/sys/util/stringUtil.js" ></script>	
<script>
...
<c:choose>
<c:when test="${(pageName=='PrivateWorkVeiw' or pageName =='reply') and (userLoginInfo.permission > '00019'  or  userLoginInfo.name==statsList[0].rgNm)}">
...
<div class="popUpMenu title_area offerL" id="offerPr" style="display:block;z-index:100;">
	...
	<c:forEach var="oFFer" items="${statsList}" varStatus="status">
		...
		<c:choose>
			<c:when test="${(pageName=='PrivateWorkVeiw' or pageName =='reply') and (userLoginInfo.permission > '00019'  or  userLoginInfo.name==oFFer.rgNm)}"><c:set var="update_view" value="update"/><!-- update --></c:when>
		...
		<c:when test="${oFFer.offerCd=='00002'}"><!-- 정보.IR -->
			<c:set var="IW" 	value="Y"/>
			<c:set var="CP1" 	value="Y"/>			<!-- 회사 라벨 -->
			<c:set var="CP3" 	value="Y"/>			<!-- 물건 라벨 -->
			<c:set var="CP_BT" 	value="Y"/>
			...
			<%-- <c:set var="noneCpn" value="Y"/> 			20161219 sjy 정보 - 탐방, IR 회사 선택 추가 --%>		
		</c:when>
	...
	<ul>
		...
		<c:if test="${not empty oFFer.middleOfferCd and not empty oFFer.offerCd }">
			...
			<select class="work-1stSelect" onchange="slctWork(this,'1')" <c:if test="${update_view == 'view'}">disabled="true"</c:if>><!-- 딜 -->
				<c:forEach var="cmmCd" items="${cmmCd1stSlctList}"><c:if test="${cmmCd.cdSort == '00003'}">
			...
			<select class="work-2ndSelect" onchange="slctWork(this,'2')" <c:if test="${update_view == 'view'}">disabled="true"</c:if> <c:if test="${slct1st != '00010'}">style="display:none"</c:if>><!-- 중개 -->
				<c:forEach var="cmmCd" items="${cmmCdDealList}"><c:if test="${cmmCd.cdSort == '00000'}">
					<option value="${cmmCd.dTailCd}" <c:if test="${oFFer.middleOfferCd == cmmCd.dTailCd}">selected</c:if>>${cmmCd.cdNm}</option>
				</c:if></c:forEach>
			</select>
			<select class="work-2ndSelect" onchange="slctWork(this,'2')" <c:if test="${update_view == 'view'}">disabled="true"</c:if> <c:if test="${slct1st != '00011'}">style="display:none"</c:if>><!-- 일임계약 -->
				<c:forEach var="cmmCd" items="${cmmCdDealList}"><c:if test="${cmmCd.cdSort == '00001'}">
					<option value="${cmmCd.dTailCd}" <c:if test="${oFFer.middleOfferCd == cmmCd.dTailCd}">selected</c:if>>${cmmCd.cdNm}</option>
				</c:if></c:forEach>
			</select>
			<select class="work-2ndSelect" onchange="slctWork(this,'2')" <c:if test="${update_view == 'view'}">disabled="true"</c:if> <c:if test="${slct1st != '00012'}">style="display:none"</c:if>><!-- 미팅 -->
				<c:forEach var="cmmCd" items="${cmmCdOfferList}"><c:if test="${cmmCd.cdSort == '00000'}">
					<option value="${cmmCd.dTailCd}" <c:if test="${oFFer.offerCd == cmmCd.dTailCd}">selected</c:if>>${cmmCd.cdNm}</option>
				</c:if></c:forEach>
			</select>
			<select class="work-2ndSelect" onchange="slctWork(this,'2')" <c:if test="${update_view == 'view'}">disabled="true"</c:if> <c:if test="${slct1st != '00015'}">style="display:none"</c:if>><!-- 회원사 -->
				<c:forEach var="cmmCd" items="${cmmCdDealList}"><c:if test="${cmmCd.cdSort == '00007'}">
					<option value="${cmmCd.dTailCd}" <c:if test="${oFFer.middleOfferCd == cmmCd.dTailCd}">selected</c:if>>${cmmCd.cdNm}</option>
				</c:if></c:forEach>
			</select>
			<select class="work-3rdSelect" onchange="slctWork(this,'3')" disabled="true" <c:if test="${slct1st != '00010'}">style="display:none"</c:if>><!-- 제안 -->
				<c:forEach var="cmmCd" items="${cmmCdOfferList}"><c:if test="${cmmCd.cdSort == '00001'}">
					<option value="${cmmCd.dTailCd}" <c:if test="${oFFer.offerCd == cmmCd.dTailCd}">selected</c:if>>${cmmCd.cdNm}</option>
				</c:if></c:forEach>
			</select>
			<select class="work-3rdSelect" onchange="slctWork(this,'3')" <c:if test="${update_view == 'view'}">disabled="true"</c:if> <c:if test="${slct1st != '00015'}">style="display:none"</c:if>><!-- 직접섭외 -->
				<c:forEach var="cmmCd" items="${cmmCdOfferList}"><c:if test="${cmmCd.cdSort == '00002'}">
					<option value="${cmmCd.dTailCd}" <c:if test="${oFFer.offerCd == cmmCd.dTailCd}">selected</c:if>>${cmmCd.cdNm}</option>
				</c:if></c:forEach>
			</select>
			<select class="work-3rdSelect" onchange="slctWork(this,'3')" <c:if test="${update_view == 'view'}">disabled="true"</c:if> <c:if test="${oFFer.offerCd != '00010'}">style="display:none"</c:if>><!-- 산업분석 -->
				<c:forEach var="cmmCd" items="${cmmCdanalysisCpnList}">
					<option value="${cmmCd.dTailCd}" <c:if test="${oFFer.cpnTypeCd == cmmCd.dTailCd}">selected</c:if>>${cmmCd.cdNm}</option>
				</c:forEach>
			</select>
			<span id='CateCd_${cnt}' <c:if test="${slct1st != '00010' and slct1st != '00013'}">style="display:none"</c:if>><!-- &nbsp;유형: -->
				<select id='categoryCd_' <c:if test="${update_view == 'view'}">disabled="true"</c:if> onchange="chngCategoryCd(this);">
					...
					<option value="${cmmCd.dTailCd}" <c:if test="${oFFer.categoryCd == cmmCd.dTailCd}">selected</c:if>>${cmmCd.cdNm}</option></c:forEach>
				</select>
		...
		<c:if test="${update_view == 'update'}">
			...
			<input type="hidden" id="AP_cpnListed_${cnt}" value="${oFFer.listed }"/>
			...
			<span id="slctCpn_${cnt}" <c:if test="${noneCpn=='Y'}">style="display:none;"</c:if>><b>
				...
				<span id="textTmp_${cnt}"   <c:if test="${CP3!='Y'}">style="display:none"</c:if>><small>(물건)</small></span></b>
				 : <span class="btn s orange">
					...
					<c:if test="${update_view == 'view'  }"><a class="c_title" onclick="popUp('','rcmdCpn','','${oFFer.cpnSnb}');"></c:if>
					${oFFer.cpnNm} 
				</a></span>
			</span>
			</nobr>
				...
				<nobr>
					...
					<span id="cstTitle5_${cnt}" <c:if test="${CS5!='Y'}">style="display:none"</c:if>>
						...
						<b>고객</b>
							...
							<c:if test="${update_view == 'view'}"  ><a class="c_title" title="${oFFer.position}" onclick="slctCst_this('${oFFer.cstId}');"></c:if>
		...
		<c:if test="${update_view == 'update'}">
			...
			<c:if test="${not empty oFFer.mediInfo}">
				<span style="float:right;color:blue;">※중개: ${oFFer.mediInfo}</span>
			</c:if>
		</li>
		...
		<c:if test="${update_view == 'update'}">
				...
				<li>
					...
					<c:forEach var="Ofif" items="${offerInfoList}" varStatus="OfifS"><c:if test="${Ofif.cdSort eq '00001'}"><c:set var="kpCnt" value="${kpCnt+1}"/>
						...
						<span class="keyPstl <c:if test="${Ofif.categoryCd eq '00008'}">wide_analysis</c:if>">
							<label><input type="checkbox" id="chk${fn:substring(Ofif.categoryCd,3,6)}_0${cnt}" <c:if test="${Ofif.categoryCd eq '00008' and (isExistAnalOpt eq 'Y' or isToday ne 'Y') and userLoginInfo.analMaster ne 'Y'}">disabled="true"</c:if> value="Y" class="input_chk" onclick="chkBoxViewTextarea(this)"<c:if test="${not empty Ofif.comment}"> checked</c:if>/><c:choose><c:when test="${empty Ofif.cdDc}">${Ofif.cdNm}</c:when><c:otherwise>${Ofif.cdDc}</c:otherwise></c:choose></label>
							<c:if test="${Ofif.categoryCd eq '00008'}">&nbsp;
							<c:forEach var="starC" begin="1" end="${Ofif.star}"><img class="hand" id="file${starC}" src="../images/figure/star_y.png" style="width:16px;height:16px;vertical-align:middle;" <c:if test="${(isExistAnalOpt ne 'Y' and isToday eq 'Y') or userLoginInfo.analMaster eq 'Y'}"> onmousedown="javascript:slctStar(this);"</c:if> /></c:forEach>
							<c:forEach var="starC" begin="${Ofif.star+1}" end="5"><img class="hand" id="file${starC}" src="../images/figure/star_g.png" style="width:16px;height:16px;vertical-align:middle;" <c:if test="${(isExistAnalOpt ne 'Y' and isToday eq 'Y') or userLoginInfo.analMaster eq 'Y'}"> onmousedown="javascript:slctStar(this);"</c:if> /></c:forEach>
							
							<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(※분석 유효기간 : 
							<label><input type="radio" <c:if test="${(isExistAnalOpt eq 'Y' or isToday ne 'Y') and userLoginInfo.analMaster ne 'Y'}">disabled="true"</c:if> name="expirationDate" value="3" <c:if test="${Ofif.expirationDt eq '3'}">checked</c:if>><c:if test="${Ofif.expirationDt eq '3'}"><u></c:if>3개월 <c:if test="${Ofif.expirationDt eq '3'}"></u></c:if></label>
							<label><input type="radio" <c:if test="${(isExistAnalOpt eq 'Y' or isToday ne 'Y') and userLoginInfo.analMaster ne 'Y'}">disabled="true"</c:if> name="expirationDate" value="6" <c:if test="${Ofif.expirationDt eq '6'}">checked</c:if>><c:if test="${Ofif.expirationDt eq '6'}"><u></c:if>6개월 <c:if test="${Ofif.expirationDt eq '6'}"></u></c:if></label>
							<label><input type="radio" <c:if test="${(isExistAnalOpt eq 'Y' or isToday ne 'Y') and userLoginInfo.analMaster ne 'Y'}">disabled="true"</c:if> name="expirationDate" value="9" <c:if test="${Ofif.expirationDt eq '9'}">checked</c:if>><c:if test="${Ofif.expirationDt eq '9'}"><u></c:if>9개월 <c:if test="${Ofif.expirationDt eq '9'}"></u></c:if></label>
							<label><input type="radio" <c:if test="${(isExistAnalOpt eq 'Y' or isToday ne 'Y') and userLoginInfo.analMaster ne 'Y'}">disabled="true"</c:if> name="expirationDate" value="12" <c:if test="${Ofif.expirationDt eq '12'}">checked</c:if>><c:if test="${Ofif.expirationDt eq '12'}"><u></c:if>1년 <c:if test="${Ofif.expirationDt eq '12'}"></u></c:if></label>
							<label><input type="radio" <c:if test="${(isExistAnalOpt eq 'Y' or isToday ne 'Y') and userLoginInfo.analMaster ne 'Y'}">disabled="true"</c:if> name="expirationDate" value="24" <c:if test="${Ofif.expirationDt eq '24'}">checked</c:if>><c:if test="${Ofif.expirationDt eq '24'}"><u></c:if>2년 <c:if test="${Ofif.expirationDt eq '24'}"></u></c:if></label>
							)
							
							</c:if></span>
							<textarea id="kyPoint${fn:substring(Ofif.categoryCd,3,6)}_0${cnt}" <c:if test="${Ofif.categoryCd eq '00008' and (isExistAnalOpt eq 'Y' or isToday ne 'Y') and userLoginInfo.analMaster ne 'Y'}">readonly="true"</c:if> class="d_note" placeholder="NEEDS 내용"<c:if test="${empty Ofif.comment }"> style="display:none"</c:if>><c:if test="${not empty Ofif.comment }">${Ofif.comment}</c:if></textarea>
			...
			<li style="padding-top:7px;">
				<span style="float:left;">
					<span id="showTabDiv">
						<c:forEach items="${cmmCdTabList}" var="item" varStatus="itemIndex">
							<span class="tab_t tab_${item.dTailCd }"> ${item.cdNm} </span>
						</c:forEach>
					</span>
				</span>
				<span style="float:right;margin-right:15px;">
					<span class="help_message" onmouseover="showTool()" onmouseout="hideTool()"> 도움말 </span>
					<span class="btn s gold"><a href="#" onclick="resizeMemo('txtarea${cnt}');return false;"><i class="fa fa-arrow-circle-down"></i></i> 펼쳐보기</a></span>
				</span>
			</li>
			<li id="tooltip1" style="display:none;">
				<div class="tooltip1">
				    <b>(1) 벤처</b> <br/>
				    	<span style="padding-left:5px;">- 설립단계부터 상장까지 3~4년 이상 남은 stage의 매출규모가 상대적으로 적은 기업</span><br/>
					<b>(2) 메자닌-비상장</b> <br/>
						<span style="padding-left:5px;">- 상장 2~3년 남은 stage, 메자닌 중심 </span></br>
					<b>(3) 프리 IPO</b> <br/>
						<span style="padding-left:5px;">- 상장 직전 stage, 구주 중심 </span></br>
				</div>
			</li>
		...
		<li class="bsnsR_btn">
			<c:if test="${update_view == 'update'}">
				...
				<span id="btn_delete" class="offerR_btnDel btn s red" style="<c:if test="${isExistAnalOpt eq 'Y' and userLoginInfo.analMaster ne 'Y'}">display:none;</c:if>"><a>완전삭제</a></span>				
			</c:if>
	...
	<c:if test="${empty statsList}">
		...
		<ul>
			...
			<!-- 소개창 -->
				...
				<input type="hidden" id="AP_cpnListed_0${cnt}" value=""/>
				...
				<nobr><span id="slctCpn_${cnt}"><b>
					...
					<span id="cpnTYPE" style="display:none">업종:
						<select id="cpnTYPEcd">
							<c:forEach var="cmmCd" items="${cmmCdTypeList}">
								<option value="${cmmCd.dTailCd}"<c:if test="${'00009' == cmmCd.dTailCd}">selected</c:if>>${cmmCd.cdNm}</option>
							</c:forEach>
						</select>
				...
				<ul style="padding: 1px;font-family: 맑은 고딕, Trebuchet MS, Verdana, Geneva, sans-serif;">
					<li><input type="hidden" id="kyPmaxLeng" value="${fn:length(cmmCdKeyPointList)}"/>
						<c:forEach var="kp" items="${cmmCdKeyPointList}" varStatus="kpS">
							<c:if test="${kp.cdSort=='00001'}"><c:set var="kpCnt" value="${kpCnt+1}"/>
								<c:if test="${kp.dTailCd eq '00008'}">
									<span id="KP_ivstCmnt${cnt}" style="display:none;"><input type="hidden" id="chkStar_0${cnt}" value=""/>
								</c:if>
								
								<span class="keyPstl <c:if test="${kp.dTailCd eq '00008'}">wide_analysis</c:if>"><%--<c:if test="${isToday ne 'Y'}"> --%>
									<label><input type="checkbox" id="chk${fn:substring(kp.dTailCd,3,6)}_0${cnt}" <c:if test="${kp.dTailCd eq '00008' and isToday ne 'Y' and userLoginInfo.analMaster ne 'Y'}">disabled="true"</c:if> value="Y" class="input_chk" onclick="chkBoxViewTextarea(this)"/>
									<c:choose><c:when test="${empty kp.cdDc}">${kp.cdNm}</c:when><c:otherwise>${kp.cdDc}</c:otherwise></c:choose>
									</label>
									<c:if test="${kp.dTailCd eq '00008'}">
										&nbsp;<c:forEach var="starC" begin="${Ofif.star}" end="4" varStatus="starCS"><img class="hand" id="file${starCS.count}" src="../images/figure/star_g.png" style="width:16px;height:16px;vertical-align:middle;" <c:if test="${isToday eq 'Y' or userLoginInfo.analMaster eq 'Y'}">onmousedown="javascript:slctStar(this);"</c:if> />
										</c:forEach>
										<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(※분석 유효기간 : 
										<label><input type="radio" <c:if test="${isToday ne 'Y' and userLoginInfo.analMaster ne 'Y'}">disabled="true"</c:if> name="expirationDate" value="3">3개월</label>
										<label><input type="radio" <c:if test="${isToday ne 'Y' and userLoginInfo.analMaster ne 'Y'}">disabled="true"</c:if> name="expirationDate" value="6">6개월</label>
										<label><input type="radio" <c:if test="${isToday ne 'Y' and userLoginInfo.analMaster ne 'Y'}">disabled="true"</c:if> name="expirationDate" value="9">9개월</label>
										<label><input type="radio" <c:if test="${isToday ne 'Y' and userLoginInfo.analMaster ne 'Y'}">disabled="true"</c:if> name="expirationDate" value="12">1년 </label>
										<label><input type="radio" <c:if test="${isToday ne 'Y' and userLoginInfo.analMaster ne 'Y'}">disabled="true"</c:if> name="expirationDate" value="24">2년 </label>
										)
									</c:if>
								</span>
								<textarea id="kyPoint${fn:substring(kp.dTailCd,3,6)}_0${cnt}" class="d_note" <c:if test="${kp.dTailCd eq '00008' and isToday ne 'Y' and userLoginInfo.analMaster ne 'Y'}">readonly="true"</c:if> style="display:none" placeholder="NEEDS 내용"></textarea>
								
								<c:if test="${kp.dTailCd eq '00008'}"></span></li><li></c:if>
			...
			<li style="padding-top:7px;">
				...
				<span style="float:right;margin-right:15px;">
						<span class="help_message" onmouseover="showTool()" onmouseout="hideTool()"> 도움말 </span>
						<span class="btn s gold"><a href="#" onclick="resizeMemo('txtarea${cnt}');return false;"><i class="fa fa-arrow-circle-down"></i> 펼쳐보기</a></span>
				</span>
			</li>
			<li id="tooltip1" style="display:none;">
				<div class="tooltip1">
...
function checkTab(){
	...
	//4가지를 모두 만족해야 탭 색상을 보여줄수 있음.
	//딜여부, middleoffercd, categorycd, 회사정보
	var checkTabYn = true;
	
	var selectInfo_1 = $("select[class='work-1stSelect']").val();
	var selectInfo_2 = $("#middleOfferCd_").val();
	var selectInfo_3 = $("#categoryCd_").val();

	//회사정보를 가져오기 위한 input id값 가져오기.(신규시 0, 업데이트시 1로 보여짐)
	var cpn = "AP_cpnId_";
	var cpnListed = "AP_cpnListed_";
	if("${cnt}" == ""){
		cpn += "0";
		cpnListed += "0";
	}else{
	...
	var selectInfo_4 = $("#"+cpn).val();
	
	var selectInfo_5 = $("#"+cpnListed).val();		//'Y':상장, 'N':비상장
	
	//첫번째 select 버튼 체크(딜: 00010)
	if("00010" != selectInfo_1){
		checkTabYn = false;
	}else{
		//딜을 선택했을 경우 탭이 다시 보이도록 수정.
		$("#showTabDiv").css("visibility","visible");
	}
	
	//두번째 select 버튼 체크(middleOfferCd)
	if(selectInfo_2 != "00001" &&  selectInfo_2 != "00002" && selectInfo_2 != "00004" && selectInfo_2 != "00005"){
		checkTabYn = false;
	}
	...
	if(checkTabYn){
		removeAllTabOn();
		if(selectInfo_2 == "00004" || selectInfo_2 == "00005"){
			//딜제안중
			$("span[class='tab_t tab_00002']").addClass("tab_t_on");
		}else{
			if(selectInfo_3 == "00001" || selectInfo_3 == "00002" || selectInfo_3 == "00003" || selectInfo_3 == "00015"){
				//메자닌인경우
				if(selectInfo_5 == 'Y'){			//selectInfo_4.substring(0,1) == 'A'){
					//상장
					$("span[class='tab_t tab_00001']").addClass("tab_t_on");
				}else{
					//비상장
					$("span[class='tab_t tab_00004']").addClass("tab_t_on");
				}
			}else if(selectInfo_3 == "00004" || selectInfo_3 == "00005" || selectInfo_3 == "00006" || selectInfo_3 == "00007"){
				//블록딜
				$("span[class='tab_t tab_00003']").addClass("tab_t_on");
					
			}else if(selectInfo_3 == "00008"){
				//M&A
				$("span[class='tab_t tab_00007']").addClass("tab_t_on");	
				
			}else if(selectInfo_3 == "00012"){
				//프리 IPO
				$("span[class='tab_t tab_00005']").addClass("tab_t_on");
				
			}else if(selectInfo_3 == "00013"){
				//스팩
				$("span[class='tab_t tab_00008']").addClass("tab_t_on");
				
			}else if(selectInfo_3 == "00016"){
				//벤처
				$("span[class='tab_t tab_00006']").addClass("tab_t_on");
			}else if(selectInfo_3 == "00017"){
				//바이오/헬스케어
				$("span[class='tab_t tab_00009']").addClass("tab_t_on");
			}
		}
	}else{
		removeAllTabOn();
		if("00010" != selectInfo_1){
			//딜이 아닌경우 무조건 해당 탭 안보이도록
			$("#showTabDiv").css("visibility","hidden");
		}
	}
}

function removeAllTabOn(){
	//모든 탭에 class 제거하기
	$(".tab_t_on").removeClass("tab_t_on");
	
	/* $("span[class='tab_t tab_00001 tab_t_on']").removeClass("tab_t_on");
	$("span[class='tab_t tab_00002 tab_t_on']").removeClass("tab_t_on");
	$("span[class='tab_t tab_00003 tab_t_on']").removeClass("tab_t_on");
	$("span[class='tab_t tab_00004 tab_t_on']").removeClass("tab_t_on");
	$("span[class='tab_t tab_00005 tab_t_on']").removeClass("tab_t_on");
	$("span[class='tab_t tab_00006 tab_t_on']").removeClass("tab_t_on");
	$("span[class='tab_t tab_00007 tab_t_on']").removeClass("tab_t_on");
	$("span[class='tab_t tab_00008 tab_t_on']").removeClass("tab_t_on"); */
}
...
function chngCategoryCd(th){
	...
	if(typeof infoLevelPop !== 'undefined'){		//infoLevelPop){
