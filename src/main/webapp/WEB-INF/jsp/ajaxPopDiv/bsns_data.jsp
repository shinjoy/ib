...
<script type="text/JavaScript">
	...
	$("#closeTab").mousedown(function(){
		$("#workPr").draggable({disabled:false, opacity:'0.8'});
	});
	$("#closeTab").mouseup(function(){
		$("#workPr").draggable({disabled:true});		
	});
	//---------- draggable :E ----------
</script>
<div>
	<c:forEach var="bsnsRcd" items="${bsnsList}" varStatus="bsnsS">
		...
		<c:choose>
			<c:when test="${userLoginInfo.permission > '00019'  or  userLoginInfo.name==bsnsRcd.name}"> 
				...
				<ul>
					<li class="c_title">
						...
						<input type="text" class="n_note" id="txt" value="${bsnsRcd.title}" style="width:500px;margin-bottom:3px;" /></li> 
