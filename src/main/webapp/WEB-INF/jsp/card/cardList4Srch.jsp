...
<c:if test="${ userLoginInfo.id eq 'kmy'
			...
			or userLoginInfo.id eq 'gjh'
			or userLoginInfo.id eq 'oys'
										}" >
		...
		<div id="wholePanel">
		    ...
		    <div id="rightPanel" style="min-height:950px;">
		        ...
		        <!--tab-->
		         <div class="gtabZone">
		            <ul>
		            	<li class="off"><a href="javascript:fnObj.goStatistics();">지출통계</a></li>		                
		            </ul>
		            <ul>
		            	<li class="on"><a href="#">지출상세검색</a></li>		                
		            </ul>
       			</div>
		        <!--//tab//-->
		        <div id="containerWrap">
		            <div class="carSearchBox">		            
		            	<div class="title">		            		
		            		...
		            		<span id="yearDiv" style="margin-left:30px;"></span>			<!-- 년도 -->
		            		<span id="monthDiv"></span>         							<!-- 월 -->
		            		<span id="overPriceDiv" style="margin-left:50px;"></span>		<!-- 초과금액 -->
		            		
		            		<c:if test="${mgmtRoleA eq 'Y'}">
		            		<span style="position:absolute;right:15px;top:11px;">
		            ...
		            <div class="widescroll_wrap" style="padding-top:10px;">
		            <b style="color:#8e99a8;padding-left:3px;"><span id="total_count"></span> 건</b>
		           	<div class="network_st_box">
		            	<span class="scroll_cover" onclick="fnObj.showAllGrid();" style="height:44px;">▼</span>
		            	  <!--================= 상단 table : S ================-->
			                    ...
			                    <table class="network_tb_st" style="table-layout:fixed;" summary="고객관리 리스트(이름, 업종, 회사, 기본정보, 네트워크, 최근정보, 시너지와의 이력)">
			                        ...
			                        <thead>
			                            <tr id="tblHeaderPart">
			                                ...
			                                <th rowspan="2" scope="col"><a href="#" onclick="fnObj.doSort(3);" id="sort_column_prefix3" class="sort_normal" style="color:white;">금액(단위:원)<em></em></a></th>
...
var fnObj = {
	...
	preloadCode: function(){
		...
		fnObj.overPrice();			//초과금액 선택
		
    	
    	//현재월 선택
    	var thisMonth = new Date().getMonth() + 1;    	
    	$('#month_'+thisMonth).addClass("select_m");
    	$('#choiceMonth').val(thisMonth);
    ...
    pageStart: function(){
    	...
    	var configObj = {
            ...
            {key:"cstNm", formatter:function(obj){														//이름
            										...
            										if(obj.item.etcNum>0){
            											txt = obj.item.cstNm+' 외 '+obj.item.etcNum+'명';
            											if(obj.item.extraName != ''){
            												txt += '</br><font style="color:black;">['+obj.item.extraName+']</font>';
            											}
            										}
            										return txt;
            									 }},
    	...
    	//-------------------------- 소팅 설정 :S ----------------------------    	
    	 mySorting.setConfig({
			colList : ['tmDt', 'dv', 'staffNm', 'price'],		//['지출일', '항목', '등록자', ''],
    ...
    doSearch: function(){ 
    	...
    	var callback = function(result){
			...
			if(overPrice == ''){
				mySorting.setSort(0);				//소팅객체를 소팅한다.(상태값들의 변화)		0 : 지출일자
			}else{
				mySorting.setSort(3);				//소팅객체를 소팅한다.(상태값들의 변화)		3 : 금액
			}
    ...
    searchTypeSetting : function(){
    	var str = '<b style="color:#3a485e;">분류 </b> <select id="dv2" style="height:26px;width:100px;" onchange="fnObj.doSearch();">';
    ...
    yearSetting : function(){
    	...
    	var str ='<b style="color:#3a485e;">년도 </b> <select id="choiceYear" style="height:26px;width:100px;" onchange="fnObj.doSearch();">';
    ...
    monthClick : function(i){
  	...
  	//초과금액 세팅(셀렉트박스)
    overPrice : function(){
    	var str ='<b style="color:red;">초과금액 :</b> <select id="overPrice" style="height:26px;width:100px;" onchange="fnObj.doSearch();">';
    	for(var i=0; i<60; i+=10){    		
    		str += '<option value="'+ (i*10000==0?'':i*10000) +'">'+ i+' 만원</option>';
    	}
    	str += '</select>';
    	$("#overPriceDiv").html(str);
    },//end yearSetting  
