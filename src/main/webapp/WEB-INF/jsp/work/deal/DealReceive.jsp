...
<input type="hidden" id="rgstId" value="<c:out value='${userLoginInfo.id}'/>">	
	...
	<section>
		...
		<form name="modifyRec" action="<c:url value='/work/selectWorkMediateDealR.do' />" method="post"><input type="hidden" name="sorting" id="sorting"><input type="hidden" name="total" value="${ttT}"></form>
		<input type="hidden" id="SendReceive" value="MR">
		
		<div class="fixed-top" style="z-index:10;">

			<header>
				<form name="dayForm" id="dayForm" method="post" action="<c:url value='/work/selectWorkMediateDealR.do' />">
			...
			<table class="t_skinR00" style="table-layout:fixed">
				<colgroup>
					<col width="77"><col width="47"><col width="102"><col width="47"><col width="47"><col width="102"><col width="102"><col width="102"><col width="52"><col width="76"><col width="88"><col width="88"><col width="76"><col width="76"><col width="180">
				</colgroup>
				<thead>
					<tr>
						...
						<th rowspan="2" onclick="sortTable(3);" class="hand bgYlw pd0">제안<br/>받은자<br/>▼</th>
				...
				<tbody>
					<c:forEach var="offer" items="${offerList}" varStatus="status">
						...
						<input type="hidden" id="offerSnb${status.count}" value="${offer.sNb}">
							...
							<td class="cent pd0"><c:forEach var="cmmCd" items="${cmmCdCategoryList}"><c:if test="${offer.categoryCd == cmmCd.dTailCd}">${cmmCd.cdNm}</c:if></c:forEach></td>
