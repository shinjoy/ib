...
<link href="<c:url value='/css/new_common.css'/>" rel="stylesheet" type="text/css">
	...
	<div class="modalWrap2">
		...
		<div class="mo_container">
			...
			<div class="carSearchBox">
				<span class="carSearchtitle">시너지</span><span id="deptClassSelectTag"></span>
			...
			<div id="dataTr" class="wrap_select_tree mgt20">
				...
				<div class="left_block">
					...
					<div class="treeShape_st" style="margin:0px 10px 10px;">
						<ul id="treeArea"></ul>					<!-- 왼쪽 트리 체크박스-->
					</div>
				</div>
				<!--/왼쪽영역/-->
...
var fnObj = {
    ...
    pageStart: function(){
    	...
    	var configObj = {
            ...
            {key:"",				formatter:function(obj){return '시너지'}},
            ...
            {key:"chk",				formatter:function(obj){
				...
				if(isUserSelectEabled != 'N' && getArrayIndexWithValue(g_disabledList,obj.item.userId)<0){
					str = '<button type="button" onclick="fnObj.delUser(' + obj.item.userId + ');return false;" class="btn_s_type_g"><em>삭제</em></button>';
				}
    ...
    doSearchNm : function(searchValue,th){
		...
		for(var i=0;i<g_userList.length;i++){
			...
			if(userName.match(searchValue)){
				...
				str +='<li onclick="javascript:fnObj.searchClick(\''+g_userList[i].userId+'\');"><a href="" class="atcmp_keyword" onclick="return false;">';
				str +='<span class="atcmp_name">'+userName.replace(new RegExp(searchValue,"gi"), '<strong>'+searchValue+'</strong>')+'</span>';
				str +='<span class="experson">(<em class="atcmp_com">시너지</em>|<em class="atcmp_team">'+g_userList[i].deptNm+'</em>|<em class="atcmp_num">'+g_userList[i].sabun+'</em>)</span></a></li>';
			}
		}
			...
			if(isDeptSelect !='Y'){
				for(var i = 0;i<list.length;i++){
					...
					if(isOneUser != 'Y'){		//단일 유저 선택이아니면,
						str+=' ><input type="checkbox" name="userCheckBox" id="userCheck_'+list[i].userId+'" value="'+list[i].userId+'" '+disabledYn+' '+userDisabled+' >';
					}else{
						str+=' id="userLabel_'+list[i].userId+'">';
					}

					str+= '<i class="icon_cir '+isIconCir+'"></i><span class="t_name">' + list[i].userName+'</span></label>';
  ...
  getUserList : function(){
		...
		var callback = function(result){
			...
			if(isDeptSelect !='Y'){
				...
				var str ='<span class="treeShape_st" style="margin:0px;"><li>';
				for(var i = 0;i<list.length;i++){
					...
					if(isOneUser != 'Y'){		//단일 유저 선택이아니면,
						str+=' style="padding-right:5px;"><input type="checkbox" class="userBox" name="userCheckBox" id="userCheck_'+list[i].userId+'" value="'+list[i].userId+'" '+disabledYn+' '+userDisabled+' >';
					}else{
						str+=' style="padding:5px;" id="userLabel_'+list[i].userId+'">';
					}

					str+= '<i class="icon_cir '+isIconCir+'"></i><span class="t_name">' + list[i].userName+'</span></label>';
					if(i%2 == 1 && i<list.length-1) str+= '</li><li>';
				...
				//-- 마지막 노드 그리드 세팅
				var tNav = $('.treeShape_st');
				tNav.find('li:last-child').addClass('last');
			}
		}
  ...
  saveCheckSet : function(){
	 ...
	 if(g_tabFlag == 'DEPT')fnObj.checkParent();						//현재 탭이 부서별일때
	 else{
		 fnObj.checkAllYn();			
	 }
	 ...
	 if(isOneUser == 'Y'){												//단일선택	
		 for(var i=0;i<g_userList.length;i++){
			 for(var k=0;k<resultList.length;k++){
				 if(resultList[k].userId ==g_userList[i].userId) $("#userLabel_"+resultList[k].userId).addClass('selectUserDis');
			 }
		 } 
	 }
  ...
  selectTabZone : function(type){
	  ...
	  fnObj.saveCheckSet();		//탭 변경 전 선택된 리스트 세팅
	  ...
	  for(var i=0; i<g_userList.length; i++){
		  ...
		  if($(th).is(":checked")){				//전체선택 체크되어있다
			  ...
			  if(isCheckDisable == 'N'){		//disable 된것 조작 불가면
				  ...
				  else $("#userCheck_"+g_userList[i].userId).prop("checked",true);
			  }else{
			 ...
			 if(isCheckDisable == 'N'){			//disable 된것 조작 불가면
				 ...
				 if($("#userCheck_"+g_userList[i].userId).is(":disabled")) continue;
				 else $("#userCheck_"+g_userList[i].userId).prop("checked",false);
			 }else{
				 $("#userCheck_"+g_userList[i].userId).prop("checked",false);
			 }
		  }
		 
		 fnObj.setSelectList(g_userList[i].userId);		//for 문을 돌며 user 하나하나 리스트에 담음
	  }
  ...
  checkDo : function(deptId,th,parentId){
	  ...
	  for(var i=0;i<checkList.length;i++){
		 fnObj.setSelectList(checkList[i].value);		//for 문을 돌며 user 하나하나 리스트에 담음
	  }
  },

  //선택될때마다 리스트에 추가한다.
  setSelectList : function(userId,type){

	  if(isOneUser != 'Y'){		//사원 다중선택
		  if($("#userCheck_"+userId).is(":checked")){		//현재 체크 누름
			  for(var i=0;i<g_userList.length;i++){
	  ...
	  else if(g_tabFlag != 'DEPT') fnObj.checkAllYn();					//사원별 탭일때, 전체 체크박스만 세팅 
  },
  ...
  delUser: function(userId){
  	...
  	if(isDeptSelect !='Y'){
  		if(isOneUser != 'Y') $("#userCheck_"+userId).prop("checked",false);
  	  	fnObj.setSelectList(userId,'del');
  	}else{
  		 resultList =[];
  		 $(".selectDeptDis").removeClass();
  		 myGrid.setGridData({				//그리드 데이터 세팅	*** 2 ***
