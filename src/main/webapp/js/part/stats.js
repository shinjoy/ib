...
//개인 통계 팝업 딜건 상세 div
function statsOfferInfo(rgName, kind, e, invest, mediateOrDirect){		//, pgCd, offerCd, infoProvCd, report, sort, supporterCd){
	var DATA;
	...
	if(sort=='100'||sort=='200') {
		...
		DATA = {
				...
				category : kind,
				invest : invest,
				mediate : mediateOrDirect == 'MEDIATE'?'Y':'',
				direct : mediateOrDirect == 'DIRECT'?'Y':''
		};
	}
	...
	$.ajax({
		type:"POST",        //POST GET
		url:"../stats/statsPrivateStatsDtl.do",     //PAGEURL
		data : DATA,
		contentType : "application/x-www-form-urlencoded; charset=UTF-8",
		timeout : 30000,       //제한시간 지정
		cache : false,        //true, false
