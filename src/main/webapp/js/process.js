...
//		$(".title").click(function(){
		...
		var options = $(this).attr('other_option').split('_');
		divId.find('[id^=dealOpinionCpnNm]').val(options[1]);		//딜 회사명
		divId.find('[id^=dealOpinionDealRgId]').val(options[2]);	//딜 등록자 usr_id
	});
...
/************************************************ company main start *****************************************************************/
	...
	$('.newCpn_btnOk').click(function(){
		...
		if(confirm("적용하시겠습니까?")){
			$.ajax({
				type:"POST",        //POST GET
				url:"../company/insertCpnAjax.do",     //PAGEURL
				data : ({
					listedYn: (chkRdo=='offerListed')?'Y':'N',
					cpnNm: cpnNm,
					ceoId: cstId,
					rgId: $('#rgstId').val()
					}),
...
*/
		...
		if(confirm("적용하시겠습니까?")){
			$.ajax({
				...
				data : ({
					...
					phn3: phn3.val(),
					rgId: $('#rgstId').val(),
					phnExposure: expo.is(":checked")?'N':'Y'
					}),
				timeout : 30000,       //제한시간 지정
				cache : false,        //true, false
				success: function whenSuccess(arg){  //SUCCESS FUNCTION
	...
	$(document).on("click",".nt_btnOk", function(){
		var title = $('#txt0').val();
...
/************************************************ person main end *****************************************************************/
/************************************************ stats main start *****************************************************************/
	$(document).on("change","#choice_monthS", function(){
...
// popUp 규격
	...
	if(flag=='c') {
		...
		if(flag=='PersonCPNnet'||flag=='workNetPersonCPNnet') sUrl+='&n2t=Y';
		else sUrl+='&n2t=N';
		w='500',h='600';
	}
		...
		sUrl+='?sNb='+snb+'&choice_year='+tmDate[0]+'&choice_monthS='+tmDate[1]+'&tmDt='+num;
		...
		num = $('#choice_year').val()+$('#choice_month').val()+num;
...
//투자사(경쟁사) 팝업화면 20170804 ksm
var mnaPopup;
function popUpInvestor(num,flag,kind,tblCnt,nm,snb){
	// popUp 규격
	var w = '740';
	var h = '740';
	var ah = screen.availHeight - 30;
	...
	if(flag=='c') {
		//sUrl = "../work/popUpCpn.do";
		sUrl = "../work/popUpInvestor.do";
		sUrl+='?f='+flag+'&n='+num+"&k="+kind+"&t="+tblCnt;
		w='500',h='700';
		// if(typeof(window.parent['allMask'])=='function'){ window.parent.allMask();}
	}
	//alert(":::::: "+sUrl);
...
function returnPopUp(rVal){	
	...
	if(flag=='c'){	
		...
		if(rVal.snb != null) {
			...
			$("#AP_cpnListed_"+num).val(rVal.listed);
			
			try{
				//탭 색상결정
				checkTab();
			}catch(e){
				//checkTab 사용하지 않는 화면에서는 그냥 넘긴다.
			}
		...
		if(($("#offerCd_").val() == '00002' || $("#offerCd_").val() == '00003')/* && $("#offerSnb").val() < 1*/){		//탐방 or IR 이면서 신규등록시에만, 인물에따른 회사세팅.
			
			$("#AP_cpnId"+num).val(rVal.cpnId);			//회사 코드
			$("#AP_cpnNm"+num).html(rVal.cpnNm);		//회사명
		...
		if($("#rtn").val()!='popUp'){
			var target = "mainFrame";
			if(typeof(parent.insertStockFirmIbYN)!='function') {parent.parent.location.reload();return;}
			if(parent.insertStockFirmIbYN()=="Y") target = "stockCstFrame";
			frm.target = target;//target frame name
		}
		// if(typeof(window.parent['delMask'])=='function'){ window.parent.delMask();}
...
///////// company left menu /////////////////////////////////////////////////////////////////////////
function slctCpn(snb,stock, popup){		// popup : 'Y' 이면 팝업으로 오픈
	if(popup == 'Y'){
		window.open("", "popup_window_cpn", "width=1000, height=700, scrollbars=auto");
		...
		var frm = document.getElementById('companyName');//sender form id
		frm.target = "popup_window_cpn";//target		
		frm.submit();
...
function popRgstCpn(){
	...
	$('#searchCpnNm4Reg').val($('#nameSearch').val());
...
/************************************************ recommend main end *****************************************************************/
/************************************************ person main start *****************************************************************/
function selectLvCd(net, cd, snb, frmNm,e,th){
	var obj = $(th),
		lvcd = "0000"+cd,
		s_nb = $("#cst_snb").val(),
		flag = net,
		bgTab = obj.parent().parent(),
		bgG = bgTab.find('[class*=bgLightGray]').size();

		if(cd==bgG){
		}else if(cd>bgG){
			for(var i = bgG; i < cd; i++){
				bgTab.find('[class*=tabImportant]:eq('+i+')').addClass('bgLightGray');
			}
		}else if(cd<bgG){
			for(var i = (bgG-1); i >= cd; i--){
				bgTab.find('[class*=tabImportant]:eq('+i+')').removeClass('bgLightGray');
			}
		}

	var DATA = {sNb:snb, lvCd:lvcd, flag: flag};
	var url = "../person/saveLvCd.do";
	var fn = function(arg) {
		
		if(net ==="NP" && parseInt(cd)>2 && parseInt(arg)>0){
			$("#AP_cstId_0").val($("#snb2"+snb).val());
			$("#NPrgId").val($("#snb1"+snb).val());
			$("#NPrgNm").html($("#npnm"+snb).val());
			//view('tbl00',e);				20160318 친밀도 수정시 직원과의 인맥시 뜨는 직원네트워크 정보 입력창...안뜨도록...추후 삭제예정
		}else{
			//$("#s_Name").val(s_nb);
			//$("#c_Name").val(s_nb);
			//$("#"+frmNm+"").submit();
